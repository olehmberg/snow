 ...
 *
 */
public class TableCellColumnBlockingKeyGenerator extends BlockingKeyGenerator<MatchableTableRow, MatchableTableRow, MatchableTableColumn> {
	...
	public void generateBlockingKeys(MatchableTableRow record,
			Processable<Correspondence<MatchableTableRow, Matchable>> correspondences,
			DataIterator<Pair<String, MatchableTableColumn>> resultCollector) {
		
		for(MatchableTableColumn c : record.getSchema()) {
			
			Object v = record.get(c.getColumnIndex());
			
			if(v==null) {
				v = "null";
			}
			
			resultCollector.next(new Pair<String, MatchableTableColumn>(v.toString(), c));
		}
