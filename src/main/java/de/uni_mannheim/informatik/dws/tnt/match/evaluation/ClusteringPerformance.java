...
public class ClusteringPerformance {
	...
	public Performance getOverallPerformanceFromPerformanceByCluster() {
		BigDecimal created = BigDecimal.ZERO;
		BigDecimal correct = BigDecimal.ZERO;
		BigDecimal totalCorrect = BigDecimal.ZERO;

		for(Performance perf : performanceByCluster.values()) {
			if(perf instanceof BigPerformance) {
				BigPerformance p = (BigPerformance)perf;
				created = created.add(p.getNumberOfPredicedBig());
				correct = correct.add(p.getNumberOfCorrectlyPredictedBig());
				totalCorrect = totalCorrect.add(p.getNumberOfCorrectTotalBig());
			} else {
				created = created.add(new BigDecimal(perf.getNumberOfPredicted()));
				correct = correct.add(new BigDecimal(perf.getNumberOfCorrectlyPredicted()));
				totalCorrect = totalCorrect.add(new BigDecimal(perf.getNumberOfCorrectTotal()));
			}
		}

		return new BigPerformance(correct, created, totalCorrect);
	}
	...
	public String format() {
    	...
    	if(correspondencePerformance!=null) {
			...
			sb.append(String.format("\tcorrect: %d / created: %d / in gs: %d\n", p.getNumberOfCorrectlyPredicted(), p.getNumberOfPredicted(), p.getNumberOfCorrectTotal()));
	    	sb.append(String.format("\t%s %.10f\n", org.apache.commons.lang3.StringUtils.rightPad("Precision:",10), p.getPrecision()));
	    	sb.append(String.format("\t%s %.10f\n", org.apache.commons.lang3.StringUtils.rightPad("Recall:",10), p.getRecall()));
	    	sb.append(String.format("\t%s %.10f\n", org.apache.commons.lang3.StringUtils.rightPad("F1:",10), p.getF1()));
    	} else {
