 ...
 *
 */
public class NFSWebTableNormaliser {
	...
	public Collection<Table> normaliseTo3NF(Table tableToNormalise, int nextTableId, TableColumn foreignKey, boolean addFullClosure, Set<TableColumn> nullFreeSubscheme) throws Exception {
		...
		for(Collection<TableColumn> det : t.getSchema().getFunctionalDependencies().keySet()) {
			...
			Collection<TableColumn> closure = NFSFunctionalDependencyUtils.closure(new HashSet<>(det), t.getSchema().getFunctionalDependencies(), nullFreeSubscheme);
		...
		for(int i=0; i<violationList.size(); i++) {
			...
			Set<TableColumn> v1Closure = NFSFunctionalDependencyUtils.closure(v1.getFirst(), H.getFunctionalDependencies(), nullFreeSubscheme);
		...
		for(Pair<Set<TableColumn>,Set<TableColumn>> fdToCheck : FunctionalDependencyUtils.split(Pair.fromMap(HPrime.getFunctionalDependencies()))) {
			...
			if(NFSFunctionalDependencyUtils.areEquivalentSets(newHPrimePlusJ.getFunctionalDependencies(), HplusJ.getFunctionalDependencies(), nullFreeSubscheme)) {
				HPrime = newHPrime;
			}
		}
		...
		for(Set<Set<TableColumn>> group : groups) {			
			...
			nf.getSchema().setCandidateKeys(NFSFunctionalDependencyUtils.listCandidateKeys(nf,decompositonNFS));
