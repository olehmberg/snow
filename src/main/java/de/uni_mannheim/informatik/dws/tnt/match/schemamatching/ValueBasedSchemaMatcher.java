...
public class ValueBasedSchemaMatcher {
	...
	public Processable<Correspondence<MatchableTableColumn, Matchable>> run(DataSet<MatchableTableRow, MatchableTableColumn> records) {
		...
		// remove correspondences between columns of the same table
		schemaCorrespondences = schemaCorrespondences.where((c)->c.getFirstRecord().getDataSourceIdentifier()!=c.getSecondRecord().getDataSourceIdentifier());
