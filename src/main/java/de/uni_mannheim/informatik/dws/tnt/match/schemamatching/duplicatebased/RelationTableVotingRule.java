 ...
 *
 */
public class RelationTableVotingRule extends VotingMatchingRule<MatchableTableColumn, MatchableTableRow> {
	 ...
	 */
	public RelationTableVotingRule(double finalThreshold, SimilarityMeasure<Double> numericSimilarity, SimilarityMeasure<LocalDateTime> dateSimilarity, SimilarityMeasure<String> stringSimilarity) {
	...
	public double compare(MatchableTableColumn record1, MatchableTableColumn record2,
		...
		if(logVotes && similarity>=getFinalThreshold()) {
			System.out.println(String.format("[Vote] %s votes for %s <-> %s (%f)", correspondence.getFirstRecord().get(0), record1, record2, similarity));
		}
	...
	protected double compareNumeric(Object value1, Object value2) {
		...
		Collection<Double> leftValues = null;
		
		if(value1.getClass().isArray()) {
			leftValues = new HashSet<>();
			for(Object v : (Object[])value1) {
				leftValues.add((Double)v);
			}
		} else {
			leftValues = Q.toSet((Double)value1);
		}
		...
		for(Double v1 : leftValues) {
			for(Double v2 : rightValues) {
				similarity = Math.max(similarity, numericSimilarity.calculate(v1, v2));
			}
		}
	...
	protected double compareDate(Object value1, Object value2) {
		Collection<LocalDateTime> leftValues = null;
		Collection<LocalDateTime> rightValues = null;
		
		if(value1.getClass().isArray()) {
			leftValues = new HashSet<>();
			for(Object v : (Object[])value1) {
				leftValues.add((LocalDateTime)v);
			}
		} else {
			leftValues = Q.toSet((LocalDateTime)value1);
		}
		...
		double similarity = 0.0;
		for(LocalDateTime v1 : leftValues) {
			for(LocalDateTime v2 : rightValues) {
				similarity = Math.max(similarity, dateSimilarity.calculate(v1, v2));
			}
		}
	...
	protected double compareString(Object value1, Object value2) {
		Collection<String> leftValues = null;
		
		if(value1.getClass().isArray()) {
			leftValues = new HashSet<>();
			for(Object v : (Object[])value1) {
				// pre-process value
				String stringValue1 = v.toString();
			...
			stringValue1 = removePattern.matcher(stringValue1).replaceAll("");
			stringValue1 = WebTablesStringNormalizer.normalise(stringValue1, false);
			leftValues = Q.toSet(stringValue1);
		}
		...
		double similarity = 0.0;
		for(String v1 : leftValues) {
			for(String v2 : rightValues) {
				similarity = Math.max(similarity, stringSimilarity.calculate(v1, v2));
			}
		}
