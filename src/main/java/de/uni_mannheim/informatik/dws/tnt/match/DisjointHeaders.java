...
public class DisjointHeaders {
	...
	public static DisjointHeaders fromTables(Collection<Table> tables) {
		return fromTables(tables, false);
	}

	public static DisjointHeaders fromTables(Collection<Table> tables, boolean excludeUnionContextColumns) {
    		...
    		for(int i = 0; i < t.getColumns().size(); i++) {
    			...
    			if(!c1.getHeader().equals("null") && !c1.getHeader().isEmpty()) {
	    			...
	    			for(int j = 0; j < t.getColumns().size(); j++) {
	    				...
	    				if(
	    						!c2.getHeader().equals("null") 
	    						&& !c2.getHeader().isEmpty() 
	    						&& !c1.getHeader().equals(c2.getHeader())
	    						&& (
									!excludeUnionContextColumns 
									|| 
									!(ContextColumns.isUnionContextColumn(c1.getHeader()) || ContextColumns.isUnionContextColumn(c2.getHeader()))) 	    // and if neither of the columns is a context column
									// !(ContextColumns.isContextColumnIgnoreCase(c1.getHeader()) || ContextColumns.isContextColumnIgnoreCase(c2.getHeader()))) 	    // and if neither of the columns is a context column
	    						) {
